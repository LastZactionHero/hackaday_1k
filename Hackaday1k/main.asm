...
start:
	...
	; UART
	; UBRRN = (fosc / 16 * baud) - 1 = 51; hex 33
	ldi r16, 0x00
	sts UBRR0H, r16
	ldi r16, 0x33
	sts UBRR0L, r16
...
uart_loop_store_character:
	...
	pop r17
	inc r17 ; next position in message buffer

	rjmp loop

; Copy a byte to the message buffer
;
; r16 - character to write
; r17 - current position in buffer
save_char_to_message_buffer:
	...
	inc YL
save_char_to_message_buffer_store_byte:
	st Y+, r16
	ldi r16, 0x00
	st Y+, r16
	ret
...
text_load_character:
	...
	; Seek to the start of the character
	; Loop to the position of the ASCII character
	; Subtract from the starting character
	; Then loop over program memory by adding the width (number of bytes) each time
	lpm r17, Z+
...
text_load_character_seek_next:
	...
	adc ZL, r17
	brcc text_load_character_seek_next
	inc ZH
	rjmp text_load_character_seek_next

text_load_character_width:
	mov r16, r17 ; save the character width
